<div class="global-content">
	<div class="global-section">
			<nav class="wrap">
					<header>
							<div class="header-logo">
									<h1 class="is-logo">
											<a href="https://blog.rocketseat.com.br">
													<img src="./img/blogicon.png" alt="Logo"/>
											</a>
									</h1>
							</div>

							<div class="header-nav">
									<nav class="nav-wrap">
											<ul class="nav-list">
													<li class="nav-list-item">
															<a href="https://blog.rocketseat.com.br" class="nav-link">LOGIN</a>
															<span class="nav-dot"></span>
													</li>
											</ul>
									</nav>
							</div>
					</header>
			</nav>

			<main class="wrap">

			</main>

			<div class="container">
					<section class="content-blog">
							<div class="row">
									<div class="col-md-6 col-lg-6 col-sm-6 padding-card">
											<div class="card" id="post-33537">
													<div class="row">
															<div class="col-md-5 wrapthumbnail">
																	<a href="#">
																			<div class="thumbnail" style="background-image:url(./img/linux.png);">
																			</div>
																	</a>
															</div>
															<div class=" col-md-7">
																	<div class="card-block">
																			<h2 class="card-title">
																					<a href="#">Making CSS animations using a sprite sheet</a>
																			</h2>
																			<span class="card-text d-block">Here, you can learn how to create your own sprite sheet and animate it using CSS, a method often...</span>
																			<div>
																					<div class="wrapfooter">
																							<span class="meta-footer-thumb">
																									<a href="https://blog.logrocket.com/author/hannahgooding/">
																									<img alt="" src="https://i0.wp.com/blog.logrocket.com/wp-content/uploads/2021/01/Hannah_Gooding.png?resize=150%2C150&amp;ssl=1" class="avatar avatar-40 photo jetpack-lazy-image jetpack-lazy-image--handled" height="40" width="40" data-lazy-loaded="1" loading="eager"></a>
																							</span>
																							<span class="author-meta">
																									<span class="post-name">
																									<a href="https://blog.logrocket.com/author/hannahgooding/">Hannah Gooding</a></span><br>
																									<span class="post-date">Feb 3, 2021</span>
																							</span>
																							<span class="post-read-more">
																									<a href="https://blog.logrocket.com/making-css-animations-using-a-sprite-sheet/" title="">
																									</a>
																							</span>
																					</div>
																			</div>
																	</div>
															</div>
													</div>
											</div>
									</div>
									<div class="col-md-6 col-lg-6 col-sm-6 padding-card">
											<div class="card" id="post-33537">
													<div class="row">
															<div class="col-md-5 wrapthumbnail">
																	<a href="#">
																			<div class="thumbnail" style="background-image:url(./img/linux.png);">
																			</div>
																	</a>
															</div>
															<div class=" col-md-7">
																	<div class="card-block">
																			<h2 class="card-title">
																					<a href="#">Making CSS animations using a sprite sheet</a>
																			</h2>
																			<span class="card-text d-block">Here, you can learn how to create your own sprite sheet and animate it using CSS, a method often...</span>
																			<div>
																					<div class="wrapfooter">
																							<span class="meta-footer-thumb">
																									<a href="https://blog.logrocket.com/author/hannahgooding/">
																									<img alt="" src="https://i0.wp.com/blog.logrocket.com/wp-content/uploads/2021/01/Hannah_Gooding.png?resize=150%2C150&amp;ssl=1" class="avatar avatar-40 photo jetpack-lazy-image jetpack-lazy-image--handled" height="40" width="40" data-lazy-loaded="1" loading="eager"></a>
																							</span>
																							<span class="author-meta">
																									<span class="post-name">
																									<a href="https://blog.logrocket.com/author/hannahgooding/">Hannah Gooding</a></span><br>
																									<span class="post-date">Feb 3, 2021</span>
																							</span>
																							<span class="post-read-more">
																									<a href="https://blog.logrocket.com/making-css-animations-using-a-sprite-sheet/" title="">
																									</a>
																							</span>
																					</div>
																			</div>
																	</div>
															</div>
													</div>
											</div>
									</div>
							</div>
					</section>
			</div>

			<div class="bottompagination">
					<span class="navigation">
							<li class="disabled">
									<a href="https://blog.logrocket.com/" title="previous" aria-label="previous"><i class="fa fa-angle-double-left"></i>
									</a>
							</li>
							<li class="active">
									<a class="active" href="https://blog.logrocket.com/">1</a>
							</li>
							<li>
									<a href="https://blog.logrocket.com/page/2/">2</a>
							</li>
							<li>
									<a href="https://blog.logrocket.com/page/3/">3</a>
							</li>
							<li>
									<a href="https://blog.logrocket.com/page/4/">4</a>
							</li>
							<li>
									<a href="https://blog.logrocket.com/page/2/" title="next" aria-label="next"><i class="fa fa-angle-double-right"></i>
									</a>
							</li>
					</span>
			</div>
	</div>
</div>
